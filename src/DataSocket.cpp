    ...
    while (!stoken.stop_requested()) {
        ...
        if (ret > 0) {
            ...
            if (pfds[0].revents & POLLIN) {
                ...
                while (offset < static_cast<size_t>(bytes_read)) {
                    ...
                    raw_frame.insert(raw_frame.end(), rx_buffer.begin() + offset, rx_buffer.begin() + offset + payload_size);
                    ...
                    while (it != rx_stream_accum.end()) {
                        ...
                        if (!rx_encoded.empty()) {
                            ...
                            if (decoded_frame.size() >= sizeof(ModemHeader) + sizeof(uint32_t)) {
                                ...
                                if (rx_crc == calc_crc) {
                                    std::cout << "[RX] Valid Frame -> Type: 0x0" << (int)rx_header.frame_type 
                                              << " | Seq: " << (int)rx_header.seq_num 
                                              << " | Len: " << rx_header.payload_len << "\n";
                                    
                                    // Send valid payload to the chat app!
                                    write(client_fd, decoded_frame.data() + sizeof(ModemHeader), rx_header.payload_len);
                                } else {
        ...
        if (!tx_queue_.empty()) {
            ...
            if (threshold_met || timeout_met) {
                std::cout << "[MAC] Flushing TX queue (" << tx_queue_.size() << " frames). "
                ...
                // burst_payload.insert(burst_payload.end(), preamble.begin(), preamble.end());
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(50)); // ALSA Drain
