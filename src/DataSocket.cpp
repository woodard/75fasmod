...
DataSocket::DataSocket(const std::string &socket_path, RadioController &radio,
  ...
  rx_pipe_[0] = -1; rx_pipe_[1] = -1;
  tx_pipe_[0] = -1; tx_pipe_[1] = -1;
  active_client_fd_ = -1;
}
...
void DataSocket::stop() {
  ...
  if (rx_pipe_[0] >= 0) { close(rx_pipe_[0]); rx_pipe_[0] = -1; }
  if (rx_pipe_[1] >= 0) { close(rx_pipe_[1]); rx_pipe_[1] = -1; }
  if (tx_pipe_[0] >= 0) { close(tx_pipe_[0]); tx_pipe_[0] = -1; }
  if (tx_pipe_[1] >= 0) { close(tx_pipe_[1]); tx_pipe_[1] = -1; }
  if (server_fd_ >= 0) { close(server_fd_); server_fd_ = -1; unlink(socket_path_.c_str()); }
}
...
void DataSocket::handle_client(int client_fd, std::stop_token &stoken) {
  ...
  pfds[1].fd = rx_pipe_[0]; pfds[1].events = POLLIN;
  ...
  while (!stoken.stop_requested()) {
    ...
    if (ret > 0) {
      ...
      if (pfds[0].revents & POLLIN) {
        ...
        while (offset < static_cast<size_t>(bytes_read)) {
          ...
          Frame frame(FrameType::DATA, current_seq++, payload_size, 
                      std::vector<uint8_t>(rx_buffer.begin() + offset, rx_buffer.begin() + offset + payload_size));

          std::vector<uint8_t> tx_encoded = frame.cobs_encode();
          tx_encoded.push_back(0x00);

          if (tx_queue_.empty()) queue_start_time_ = now;
      ...
      if (pfds[1].revents & POLLIN) {
        ...
        if (bytes > 0) {
          rx_stream_accum.insert(rx_stream_accum.end(), pipe_buf.begin(), pipe_buf.begin() + bytes);
          auto it = std::find(rx_stream_accum.begin(), rx_stream_accum.end(), 0x00);
          while (it != rx_stream_accum.end()) {
            ...
            if (!decoded_frame.payload.empty()) {
              ...
              write(client_fd, decoded_frame.payload.data(), decoded_frame.payload_len);
            } else {
              std::cerr << "[RX] Error: Frame failed CRC check!\n";
            }
            it = std::find(rx_stream_accum.begin(), rx_stream_accum.end(), 0x00);
          }
        }
      }
    }
    ...
    if (!tx_queue_.empty() && now >= tx_resume_time_) {
      bool threshold_met = (tx_queue_.size() >= static_cast<size_t>(burst_limit_));
      auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - queue_start_time_).count();
      ...
      if (threshold_met || timeout_met || is_transmitting_) {
        ...
        if (!is_transmitting_) {
          ...
          ptt_drop_time_ = now + std::chrono::milliseconds(80); // Hardware relay delay
          
          // Prepend 16-byte Training Preamble on first key-up
          for(int i=0; i<16; ++i) stream_chunk.push_back(0xAA); 
        }

        // Pull frames up to burst limit
        while (!tx_queue_.empty() && frames_sent_in_burst_ < burst_limit_) {
          stream_chunk.insert(stream_chunk.end(), tx_queue_.front().begin(), tx_queue_.front().end());
        ...
        // Add exact audio duration to our future unkey deadline (48000 Hz, 5 SPS, 3 bits/sym)
        double total_symbols = (stream_chunk.size() * 8.0) / 3.0;
        int duration_ms = static_cast<int>((total_symbols * 5.0 * 1000.0) / 48000.0);
        ...
        if (frames_sent_in_burst_ >= burst_limit_) {
          ...
          tx_resume_time_ = ptt_drop_time_ + std::chrono::milliseconds(300); // 300ms quiet turnaround
        }
      }
    }
  }
