...
auto THD75::set_power_level(const std::string &level) -> bool {
  ...
  for (auto &chr : lvl) chr = static_cast<char>(std::toupper(chr));
...
auto THD75::get_power_level(std::string &level) -> bool {
  ...
  switch (pwr) {
  case PowerLevel::HIGH: level = "H"; return true;
  case PowerLevel::MEDIUM: level = "M"; return true;
  case PowerLevel::LOW: level = "L"; return true;
  case PowerLevel::EXTRA_LOW: level = "EL"; return true;
  default: return false;
  }
}

auto THD75::set_other_power_level(const std::string &level) -> bool {
  if (rig_ == nullptr || get_single()) return false;
  VFO const current = get_current_vfo();
  VFO const other = (current == VFO::A) ? VFO::B : VFO::A;
  if (!set_current_vfo(other)) return false;
...
void THD75::shutdown() {
  if (rig_ != nullptr) {
    ...
    if (get_dual() && orig_other_freq_saved_) {
      ...
      VFO const other = (current == VFO::A) ? VFO::B : VFO::A;
      if (set_current_vfo(other)) {
        if (orig_other_freq_saved_) set_frequency(static_cast<double>(orig_other_frequency_) / 1e6);
        if (orig_other_mode_saved_) set_mode(static_cast<Mode>(orig_other_mode_));
        if (orig_other_power_saved_) {
          std::string level;
          switch (orig_other_power_) {
          case PowerLevel::HIGH: level = "H"; break;
          case PowerLevel::MEDIUM: level = "M"; break;
          case PowerLevel::LOW: level = "L"; break;
          case PowerLevel::EXTRA_LOW: level = "EL"; break;
          default: level = "UNKNOWN";
          }
          if (level != "UNKNOWN") set_power_level(level);
        }
        set_current_vfo(current);
      }
    }
    if (orig_menu_102_ != UsbOutSelect::unknown) kenwood_usb_out_select_set(orig_menu_102_);
    if (orig_vfo_ != RIG_VFO_NONE) rig_set_vfo(rig_, orig_vfo_);
  }
  RadioController::shutdown();
}

auto THD75::get_tnc() -> int {
  if (rig_ == nullptr) return -1;
  char cmd[] = "TN\r", buf[64] = {0};
  unsigned char term = '\r';
  int bytes = rig_send_raw(rig_, reinterpret_cast<const unsigned char *>(cmd), 3,
  ...
  if (bytes > 0 && buf[0] != '?' && buf[0] != 'N') {
    ...
    if (comma != std::string::npos && space_pos != std::string::npos) {
      try { return std::stoi(resp.substr(space_pos + 1, comma - (space_pos + 1))); } catch (...) { return -1; }
    }
  }
  return -1;
}
...
auto THD75::set_tnc(int mode) -> bool {
  ...
  unsigned char term = '\r';
  int bytes = rig_send_raw(rig_, reinterpret_cast<const unsigned char *>(cmd), strlen(cmd),
                           reinterpret_cast<unsigned char *>(buf), 63, &term);
...
auto THD75::kenwood_menu_get(int menu_num) -> int {
  ...
  if (bytes > 0 && buf[0] != '?' && buf[0] != 'N') {
    ...
    if (tpos == std::string::npos) tpos = resp.find(';');
    if (comma != std::string::npos && tpos != std::string::npos) {
      try { return std::stoi(resp.substr(comma + 1, tpos - comma - 1)); } catch (...) {}
    }
  }
  return -1;
}
...
auto THD75::kenwood_menu_set(int menu_num, int value) -> bool {
  if (rig_ == nullptr || value < 0) return false;
  char cmd[32], buf[64] = {0};
...
auto THD75::kenwood_usb_out_select_get() -> THD75::UsbOutSelect {
  ...
  if (val == 0) return UsbOutSelect::AF;
  if (val == 1) return UsbOutSelect::IF;
  if (val == 2) return UsbOutSelect::Detect;
...
auto THD75::kenwood_power_get() -> THD75::PowerLevel {
  if (rig_ == nullptr) return PowerLevel::UNKNOWN;
  ...
  if (bytes > 0 && buf[0] != '?' && buf[0] != 'N') {
    ...
    if (tpos == std::string::npos) tpos = resp.find(';');
    if (comma != std::string::npos && tpos != std::string::npos) {
      try { return static_cast<PowerLevel>(std::stoi(resp.substr(comma + 1, tpos - comma - 1))); } catch (...) {}
    }
  }
...
auto THD75::kenwood_power_set(PowerLevel val) -> bool {
  if (rig_ == nullptr || val == PowerLevel::UNKNOWN) return false;
  int active_band = (get_current_vfo() == VFO::B) ? 1 : 0;
  char cmd[32], buf[64] = {0};
...
auto THD75::get_current_vfo() -> VFO {
  if (rig_ == nullptr) return VFO::A;
  vfo_t vfo = 0;
  if (rig_get_vfo(rig_, &vfo) == RIG_OK) {
    if (vfo == RIG_VFO_B) return VFO::B;
  }
  return VFO::A;
}
...
auto THD75::get_single() -> bool {
  if (rig_ == nullptr) return false;
  int status = 0;
  if (rig_get_func(rig_, RIG_VFO_CURR, RIG_FUNC_DUAL_WATCH, &status) == RIG_OK) {
    return status == 0;
  }
  return true;
}
...
auto THD75::set_single(VFO vfo) -> bool {
  if (rig_ == nullptr) return false;
  if (!set_current_vfo(vfo)) return false;
...
auto THD75::get_other_mode() -> Mode {
  if (rig_ == nullptr || get_single()) return Mode::FM;
  VFO current = get_current_vfo();
  VFO other = (current == VFO::A) ? VFO::B : VFO::A;
  if (!set_current_vfo(other)) return Mode::FM;
  Mode mode;
  bool result = get_mode(mode);
  set_current_vfo(current);
  if (!result) return Mode::FM;
  return mode;
}
...
auto THD75::get_other_frequency() -> double {
  if (rig_ == nullptr || get_single()) return 0.0;
  VFO current = get_current_vfo();
  VFO other = (current == VFO::A) ? VFO::B : VFO::A;
  if (!set_current_vfo(other)) return 0.0;
  double freq = NAN;
