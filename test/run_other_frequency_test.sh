...
TEST_BIN="./test_other_frequency"
if [ ! -x "$TEST_BIN" ]; then
    TEST_BIN="./test/test_other_frequency"
fi

if [ ! -x "$TEST_BIN" ]; then
    echo "[ERROR] Test binary 'test_other_frequency' not found or not executable."
    exit 1
fi

echo "=== Step 1: Setting Other Frequency (-s) ==="
SET_FREQ=$( "$TEST_BIN" -s )
echo "Radio set to: ${SET_FREQ} MHz"

echo "=== Step 2: Querying Other Frequency (no flags) ==="
GET_FREQ=$("$TEST_BIN")
